...
import { Bug, ArrowRight, Lock, Mail, User } from "lucide-react";
      ...
      >
        ...
        <Tilt3DCard className="rounded-2xl" maxTilt={8}>
          <AuroraBorder className="rounded-2xl">
            ...
            >
              <h1 className="font-display text-xl font-semibold text-text mb-1">Create your account</h1>
              <p className="text-sm text-muted mb-6">Start fixing real bugs today.</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                <FloatingField icon={User} type="text" label="Name" value={name} onChange={setName} placeholder="Ada Lovelace" />
              ...
              <p className="text-xs text-muted text-center mt-6">
                Already have an account? <Link to="/login" className="text-violet hover:underline">Log in</Link>
              </p>
            </div>
          </AuroraBorder>
        </Tilt3DCard>
...
function FloatingField({ icon: Icon, type, label, value, onChange, placeholder, minLength }) {
